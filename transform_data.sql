...
SET rs.province = (
CASE WHEN EXISTS (SELECT 1 FROM province_dim AS pd 
							WHERE rs.province LIKE CONCAT('%', pd.name_vi, '%') OR
							 rs.province LIKE CONCAT('%', pd.name_en, '%') OR
							 rs.province LIKE CONCAT('%', pd.name_en, '%') OR 
							 rs.province LIKE CONCAT('%', TRIM(pd.name_vi), '%') OR 
							 rs.province LIKE CONCAT('%', TRIM(pd.name_en), '%') OR 
							 rs.province LIKE CONCAT('%', SUBSTRING_INDEX(pd.name_vi, ' ', -1), '%') OR
				    		 rs.province LIKE CONCAT('%', SUBSTRING_INDEX(pd.name_en, ' ', -1), '%')) 
			THEN (SELECT pd.id FROM province_dim AS pd 
			WHERE 
					 rs.province LIKE CONCAT('%', pd.name_vi, '%') OR
					 rs.province LIKE CONCAT('%', pd.name_en, '%') OR
					 rs.province LIKE CONCAT('%', TRIM(pd.name_en), '%') OR 
					 rs.province LIKE CONCAT('%', TRIM(pd.name_vi), '%') OR 
					 rs.province LIKE CONCAT('%', SUBSTRING_INDEX(pd.name_vi, ' ', -1), '%') OR
				    rs.province LIKE CONCAT('%', SUBSTRING_INDEX(pd.name_en, ' ', -1), '%') LIMIT 1) 
			ELSE (SELECT NULL)
 END
);

/**/
DELETE FROM records_staging WHERE province IS NULL OR province = '';
/**/
UPDATE records_staging SET time_record = IF(time_record IS NULL OR time_record = '', TIME_FORMAT(NOW(), '%H:%i:%s'), time_record) WHERE time_record IS NULL OR time_record = '' ;
/**/
UPDATE records_staging SET date_record = IF(date_record IS NULL OR date_record = '', DATE_FORMAT(NOW(), '%d-%m-%Y'), date_record) WHERE date_record IS NULL OR date_record = '';
/**/
UPDATE records_staging AS rs 
JOIN date_dim AS dd ON dd.full_date = STR_TO_DATE(rs.date_record, '%d-%m-%Y')SET rs.date_record = dd.date_key ; 
/**/
DELETE FROM records_staging WHERE time_forcast IS NULL OR time_forcast = '';
/**/
DELETE FROM records_staging WHERE date_forcast IS NULL OR date_forcast = '';
/**/
UPDATE records_staging AS rs 
JOIN date_dim AS dd ON dd.full_date = STR_TO_DATE(rs.date_forcast, '%d-%m-%Y')
...
WHERE (r1.temperature IS NULL OR r1.temperature = '') AND r1.id = (SELECT MAX(id) FROM records_staging);
 /**/
UPDATE records_staging r1
JOIN records_staging r2 ON r1.id = r2.id - 1  
JOIN records_staging r3 ON r1.id = r3.id + 1
...
WHERE (r1.feel_like IS NULL OR r1.feel_like = '') AND r1.id = (SELECT MAX(id) FROM records_staging);
/**/
...
SET rs.description = (
  CASE WHEN EXISTS (
    SELECT 1 FROM description_dim AS dd 
    WHERE rs.description = dd.name_vi
    OR rs.description =dd.name_en
  ) THEN (
    SELECT dd.id FROM description_dim AS dd
    WHERE 
      rs.description = dd.name_vi OR  
      rs.description = dd.name_en 
    LIMIT 1
  )
  ELSE (
    SELECT description FROM records_staging 
    WHERE description LIKE rs.description LIMIT 1
  )
END
);
...
/**/
UPDATE records_staging r1 SET wind_direction = (
...
CASE WHEN EXISTS (SELECT 1 FROM wind_direction_dim AS wdd 
                                                        WHERE rs.wind_direction LIKE CONCAT('%', wdd.name_vi, '%') 
                                                        OR rs.wind_direction LIKE CONCAT('%', wdd.name_en, '%')) 
                        THEN (SELECT wdd.id FROM wind_direction_dim AS wdd 
                        WHERE 
                                         rs.wind_direction LIKE CONCAT('%', wdd.name_vi, '%') OR
                                         rs.wind_direction LIKE CONCAT('%', wdd.name_en, '%')    LIMIT 1) 
                        ELSE (SELECT NULL)
 END
);
...
WHERE r1.wind_speed IS NULL OR r1.wind_speed= ''
AND r1.id <= (SELECT MAX(id) FROM records_staging);
/**/
UPDATE records_staging r1
JOIN records_staging r2 ON r1.id = r2.id - 1  
...
WHERE (r1.humidity IS NULL OR r1.humidity = '') AND r1.id = (SELECT MAX(id) FROM records_staging);
/**/
UPDATE records_staging r1
JOIN records_staging r2 ON r1.id = r2.id - 1  
JOIN records_staging r3 ON r1.id = r3.id + 1
...
WHERE (r1.uv_index IS NULL OR r1.uv_index = '') AND r1.id = (SELECT MAX(id) FROM records_staging); 

UPDATE records_staging SET uv_index =11 WHERE uv_index >11;
/**/
UPDATE records_staging r1
JOIN records_staging r2 ON r1.id = r2.id - 1  
JOIN records_staging r3 ON r1.id = r3.id + 1
...
WHERE (r1.cloud_cover IS NULL OR r1.cloud_cover = '') AND r1.id = (SELECT MAX(id) FROM records_staging);
/**/
UPDATE records_staging r1
JOIN records_staging r2 ON r1.id = r2.id - 1  
JOIN records_staging r3 ON r1.id = r3.id + 1
...
WHERE (r1.precipitation IS NULL OR r1.precipitation = '') AND r1.id = (SELECT MAX(id) FROM records_staging); 
/**/
UPDATE records_staging 
SET  accumulation=0
WHERE   accumulation IS NULL OR  accumulation = '';